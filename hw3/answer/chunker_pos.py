...
def read_conll(handle, input_idx=0, pos_idx=1, label_idx=2):
    conll_data = []
    contents = re.sub(r'\n\s*\n', r'\n\n', handle.read())
    ...
    for sent_string in contents.split('\n\n'):
        ...
        if label_idx < 0:
            conll_data.append(( annotations[input_idx], annotations[pos_idx] ))
            logging.info("CoNLL: {}".format( " ".join(annotations[input_idx])))
        else:
            assert(label_idx < len(annotations))
            conll_data.append(( annotations[input_idx], annotations[pos_idx], annotations[label_idx] ))
            logging.info("CoNLL: {} ||| {}".format( " ".join(annotations[input_idx]), " ".join(annotations[pos_idx]), " ".join(annotations[label_idx])))
    return conll_data
...
def pos_encoding(pos_tags, pos_to_ix):
    sentence_pos_vectors = list()
    for pos in pos_tags:
        p = torch.zeros(1, len(pos_to_ix))
        if pos not in pos_to_ix:
            continue
        p[0, pos_to_ix[pos]] = 1
        sentence_pos_vectors.append(p)
    return torch.stack(sentence_pos_vectors)
...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, pos_size, tagset_size):
        torch.manual_seed(1)
        ...
        self.lstm = nn.LSTM(embedding_dim + 3 * len(string.printable) + pos_size, hidden_dim, bidirectional=False)

        # The linear layer that maps from hidden state space to tag space
        self.hidden2tag = nn.Linear(hidden_dim, tagset_size)

    def forward(self, sentence, sentence_scvecotrs, pos_vecotrs):
        embeds = self.word_embeddings(sentence)
        sentence_embeddings = embeds.view(len(sentence), 1, -1)
        # sentence_scvecotrs = self.sc_encoding(sentence)
        lstm_out, _ = self.lstm(torch.cat((sentence_embeddings, sentence_scvecotrs, pos_vecotrs), -1))
...
class LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64):
        ...
        self.pos_to_ix = {} # replace pos tags with an index
        ...
        logging.info("pos_to_ix:", self.pos_to_ix)
    ...
    def argmax(self, seq, pos_tags):
        output = []
        with torch.no_grad():
            inputs = prepare_sequence(seq, self.word_to_ix, self.unk)
            pos_vectors = pos_encoding(pos_tags, self.pos_to_ix)
            tag_scores = self.model(inputs[0], inputs[1], pos_vectors)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, pos_tags, tags in tqdm.tqdm(self.training_data):
                ...
                # pos_in, _ = prepare_sequence(pos_tags, self.pos_to_ix, self.unk)
                targets, _ = prepare_sequence(tags, self.tag_to_ix, self.unk)

                # Step 3. Run our forward pass.
                pos_vectors = pos_encoding(pos_tags, self.pos_to_ix)
                tag_scores = self.model(sentence_in, sentence_scvecotrs, pos_vectors)
    ...
    def decode(self, inputfile):
        ...
        self.model.eval()
        decoder_output = []
        for sent, pos_tags in tqdm.tqdm(input_data):
            decoder_output.append(self.argmax(sent, pos_tags))
