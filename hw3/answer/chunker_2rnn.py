...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim1, hidden_dim2, vocab_size, tagset_size):
        ...
        # first rnn layer only takes char_embedding ith hiiden_dim as 150 to shrink the output size
        # so the output would be more meaningful
        self.lstm_character = nn.LSTM(3*len(string.printable), hidden_dim1, bidirectional=False)
        self.lstm_chunker = nn.LSTM(embedding_dim + hidden_dim1, hidden_dim2, bidirectional=False)
        # The linear layer that maps from hidden state space to tag space
        self.hidden2tag = nn.Linear(hidden_dim2, tagset_size)

    def forward(self, sentence, sc_vecor):
        ...
        vector = torch.cat((sentence_embeddings, sc_vecor), -1)
        # modified
        lstm_out_character, _ = self.lstm_character(sc_vecor)
        lstm_out, _ = self.lstm_chunker(torch.cat((sentence_embeddings, lstm_out_character.view(len(sentence), 1, -1)), -1))
...
class LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim1=150, hidden_dim2=64):
        self.unk = unk
        ...
        self.model = LSTMTaggerModel(self.embedding_dim, self.hidden_dim1, self.hidden_dim2, len(self.word_to_ix), len(self.tag_to_ix))
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                tag_scores = self.model(sentence_in, sc_vector)
