...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, hidden_dim_character=150):
        torch.manual_seed(1)
        ...
        self.lstm_character = nn.LSTM(3 * len(string.printable), hidden_dim_character, bidirectional=False)
        self.lstm_chunker = nn.LSTM(embedding_dim + hidden_dim_character, hidden_dim, bidirectional=False)
    ...
    def forward(self, sentence, sentence_scvecotrs):
        ...
        lstm_out_character, _ = self.lstm_character(sentence_scvecotrs)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                tag_scores = self.model(sentence_in, sentence_scvecotrs)
